...
Final models were created using the parameters of the best candidate models, inside the calibration area, performing five replicates by Bootstrap, and using the complete sets of occurrences. These models were transferred to all the Cuban archipelago and the future scenario (i.e., RCP 4.5) in the selected GCM.
...
mod1 <- raster::raster("Final_Models/M_0.7_F_lqpth_Set2_NE/Peltophryne_empusa_Cuba_median.asc")
mod2 <- raster::raster("Final_Models/M_0.8_F_lqpth_Set2_NE/Peltophryne_empusa_Cuba_median.asc")

modf1 <- raster::raster("Final_Models/M_0.7_F_lqpth_Set2_NE/Peltophryne_empusa_ccsm4_4.5_median.asc")
modf2 <- raster::raster("Final_Models/M_0.8_F_lqpth_Set2_NE/Peltophryne_empusa_ccsm4_4.5_median.asc")
...
pe_iucn <- rgdal::readGDAL("")
changes_in <- raster::mask(changes, pe_iucn)

areas <- na.omit(raster::getValues(changes_in))
...
A total of `r ress[1,1]` of `r length(resc[,1])` candidate models were significant. Since none of the candidate models resulted in omission rates lower than 5%, `r length(resb[,1])` best parameters were chosen by their AICc values (delta AICc < 2, calculated among the models with the lowest omission rates).
