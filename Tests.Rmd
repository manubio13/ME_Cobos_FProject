...
if(fea_cs == "all" | fea_cs == "basic" | fea_cs == "no.t.h" | fea_cs == "no.t.h" | fea_cs == "no.t.h"){
  if(fea_cs == "all"){
    n <- prod(length(reg_mul), length(dir("M")), 29, 2) #number of potential combinations times two (two models for each combination are being created)
  }
  if(fea_cs == "basic"){
    n <- prod(length(reg_mul), length(dir("M")), 5, 2) #number of potential combinations times two (two models for each combination are being created)
  }
  if(fea_cs == "no.t.h"){
    n <- prod(length(reg_mul), length(dir("M")), 7, 2) #number of potential combinations times two (two models for each combination are being created)
  }
  if(fea_cs == "no.t"){
    n <- prod(length(reg_mul), length(dir("M")), 15, 2) #number of potential combinations times two (two models for each combination are being created)
  }
  if(fea_cs == "no.h"){
    n <- prod(length(reg_mul), length(dir("M")), 15, 2) #number of potential combinations times two (two models for each combination are being created)
  }
...
if (sum(sdirs == fm_dir) == length(sdirs)){
  cat("\npassed")
} else{
  cat("\nfailed")
}
