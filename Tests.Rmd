...
if (file.exists("emp_enm_calibration_models1.bat")) {
  cat("\npassed")
} else {
  cat("\nfailed")
}
...
```{r cal_test_2, echo=T, cache=TRUE}
#Number of models expected to result with the defined varaibles
if(fea_cs == "all" | fea_cs == "basic" | fea_cs == "no.t.h" |)
n <- prod(length(reg_mul), length(dir("M")), 2) #number of potential combinations times two (two models for each combination are being created)
bf <- readLines("emp_enm_calibration_models1.bat") 
nr <- length(bf)
